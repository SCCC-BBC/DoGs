...
#'
convertbam2bed <- function(input.bamfile.dir, output.bedfile.dir,use.cluster=NULL)
{
    ...
    cmd.l <- lapply(1:length(res), function(u,res, output.bedfile.dir)
    {
        ...
        } else
        {
            ...
            cmd1 <- "bedtools bamtobed -i"
            cmd2 <- ">"
            cmd3 <- paste(cmd1, paste0('"',res[[u]],'"'), cmd2, paste0('"',file.path(output.bedfile.dir,
                paste0(file_name, ".bed")),'"'), sep = " ")
        }
                                 ...
                                 genome.index, cmd.input="parallel",wait.job.name=NULL)
{
  ...
  m.id <- grep("login", system("hostname", intern = TRUE))
  cmd6 = "module load python;tophat --library-type fr-firststrand -g 1 -G"
  cmd8 = "-p 4 -o"
...
generateDoGsAnnotation <- function(annotation.bed.file.dir,which.beds,
                        output.annotation.bed.file.dir,dog.length,use.cluster=NULL)
{

  annotationBed <- parserreadfiles(annotation.bed.file.dir,"bed",sample.group=which.beds)

  if (!dir.exists(output.annotation.bed.file.dir))
  {
    dir.create(output.annotation.bed.file.dir, recursive = TRUE)
  }

  print(annotationBed$input[[1]])
  file.bed <- import(annotationBed$input[[1]])

  print(file.bed)

  re1 <- as.data.frame(file.bed)
